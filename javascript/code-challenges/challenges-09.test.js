...
const updateNumbers = (obj) => {
  // Solution code here...
  return Object.keys(obj).map((item, i) => `${item}: ${Object.values(obj)[i]}`);
};
...
const getHouses = (arr) => {
  let houses = [];
  // Solution code here...
  houses = arr.map((item) => Object.values(item)[3]);
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  return ((arr.find((item) => Object.values(item)[0] === character)).children)? true : false;
};
