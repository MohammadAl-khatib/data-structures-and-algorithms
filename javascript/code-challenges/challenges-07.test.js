...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  let array = starWarsArr.sort((item1, item2) => {
...
const joinArray = (arr) => {
  // Solution code here...
  let result = arr.reduce((first, second) => {
    return first + ' ' + second;
  });
  return result;
};
...
const wordsToCharList = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    result.push(arr.substring(i, i + 1));
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((item) => {
    for (let i = 0; i < 2; i++) {
      string =item.slice(item.indexOf(' ')+1);
      string2 = string.slice(string.indexOf(' ')+1);
    }
    result.push(string2);
  });
  return result;
