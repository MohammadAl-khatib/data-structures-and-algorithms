...
function transformToLis(obj) {
  ...
  for (let i = 0; i < array1.length; i++) {
    array3.push(`<li>${array1[i]}: ${array2[i]}</li>`);
  }
  return array3;
}
...
const count = (target, input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    array.push(input[i].filter((item) => item === target));
  }
...
let findMaleAndFemale = (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i].gender === 'male'){result = result +data[i].name;}
    if (i === 1){result = result +' and ';}
    if (data[i].gender === 'female'){result = result +data[i].name;}
    if (i === 3){result = result +' and ';}
  }
  return result;
};
...
let findShortest = (data) => {
  // Solution code here...
  let result = data[0].height;
  let result2 ='';
  for (let i = 0; i < data.length; i++){
    if (Number(data[i].height) < Number (result)){
